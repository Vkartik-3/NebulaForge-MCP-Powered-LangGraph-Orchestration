...
export class AgentBuilder {
  ...
  private async callModel(state: typeof MessagesAnnotation.State) {
    ...
    console.log("[DEBUG] callModel - invoking model with", messages.length, "messages");
    const modelInvoker = this.model.bindTools(this.tools);
    try {
      const response = await modelInvoker.invoke(messages);
      console.log("[DEBUG] callModel - got response:", typeof response, response?.constructor?.name);
      console.log("[DEBUG] callModel - response content:", JSON.stringify(response, null, 2));
      return { messages: response };
    } catch (error) {
      console.error("[ERROR] callModel failed:", error);
      throw error;
    }
  }
